...
namespace server
{
	...
	public:
		CServer(const uint16_t nPortTCP, const uint16_t nPortSSL)
		{
			...
			/* Следующие две строки для нового протокола IPv6 ,пока закоментировано */
			...
			InitListenSocket(nEpoll, nPortSSL, m_ListenEventSSL, socket(AF_INET, SOCK_STREAM, 0)); /* Конец строк для протокола IPv4 */
	...
	private:
		void InitListenSocket(const int nEpoll, const uint16_t nPort, struct epoll_event &eventListen, const SOCKET listen_sd)
		{
			...
			/* Следующие пять строк для нового протокола IPv6 ,пока закоментировано */			
			...
			sa_serv.sin_addr.s_addr = htons(INADDR_ANY);
			sa_serv.sin_port = htons(nPort); /* Server Port number */ /* Конец строк для протокола IPv4 */
